...
void DLListBefore (DLList L, char *it)
{
	...
	if (L->nitems == 0) {
	    ...
	    if (L->curr->prev == NULL) {
	        ...
	        struct DLListNode *temp = L->curr->prev;
	        L->curr->prev = n;
	        n->prev = temp;
	        temp->next = n;
	        n->next = L->curr;
...
void DLListDelete (DLList L)
{
	...
	// one element
	    ...
	    struct DLListNode *dele = L->curr;
	    struct DLListNode *prev = L->curr->prev;
	    if (L->first->next == NULL) {
	         (dele);
	        L->first = L->last = L->curr = NULL;
	        L->nitems--;
	    // delete first
	    } else if (L->curr == L->first && L->first->next != NULL) {
	        L->first = L->curr->next;
	        L->curr = L->first;
	        L->first->prev = NULL;
	        freeDLListNode (dele);
	    ...
	    // delete middle
	        ...
	        prev->next = L->curr->next;
	        L->curr->next->prev = prev;
	        L->curr = L->curr->next;
	        freeDLListNode (dele);
	        L->nitems--;
	    }
